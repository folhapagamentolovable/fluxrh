...
export function OrganizationsPage() {
  ...
  const companies = useMemo(
    () =>
      data?.companies.filter((company) =>
        `${company.tradeName} ${company.legalName} ${company.document}`
          .toLowerCase()
  ...
  const submit = (event: FormEvent) => {
    ...
    if (isValidCnpj(form.document))
      mutation.mutate({ ...form, document: normalizeDigits(form.document) });
  };
  ...
  const summary = [
    ...
    ["Centros de custo", data.summary.costCenters, CircleDollarSign, "amber"],
  ] as const;
    ...
    <div className="page">
      <section className="simple-heading">
        <div>
          ...
          <p>
            Organize os vínculos que contextualizam colaboradores, regras,
            custos e permissões.
          </p>
        </div>
      ...
      <section className="org-summary">
        {summary.map(([label, value, Icon, tone]) => (
          <div className="org-summary-card" key={label}>
          ...
          <div className="company-grid">
            {companies.map((company) => (
              <article className="company-card" key={company.id}>
                ...
                <footer>
                  <span>
                    <UsersRound size={15} /> {company.employeesCount} pessoas
                  </span>
                  <span>
                    <MapPin size={15} /> {company.establishmentsCount} unidades
                  </span>
                </footer>
              </article>
            ))}
              ...
              <div className="tree-company" key={company.id}>
                ...
                <div className="tree-children">
                  {data.units
                    ...
                    .map((establishment) => (
                      <div className="tree-branch" key={establishment.id}>
                        <div className="tree-node">
                          ...
                          <span>
                            ...
                            <small>
                              ...
                              {establishment.employeesCount} pessoas
                            </small>
                          </span>
                        </div>
                        <div className="tree-children compact">
                          {data.units
                            .filter(
                              (unit) => unit.parentId === establishment.id,
                            )
                            .map((department) => (
                                ...
                                <div className="tree-node">
                                  ...
                                  <span>
                                    ...
                                    <small>
                                      ...
                                      {department.employeesCount} pessoas
                                    </small>
                                  </span>
                                </div>
                                <div className="tree-children compact">
                                  {data.units
                                    .filter(
                                      (unit) => unit.parentId === department.id,
                                    )
                                    .map((center) => (
                                      <div
                                        className="tree-node subtle"
                                        key={center.id}
                                      >
                                        <CircleDollarSign />
                                        <span>
      ...
      <Modal
        ...
        description="Inclua a pessoa jurídica principal. Unidades e departamentos poderão ser adicionados em seguida."
      >
        <form className="form-grid" onSubmit={submit}>
          ...
          {mutation.error && (
            <p className="form-error span-2">
              Não foi possível cadastrar. Revise os dados.
            </p>
          )}
          <footer className="form-actions span-2">
            ...
            <button
              ...
              disabled={mutation.isPending || !isValidCnpj(form.document)}
            >
              {mutation.isPending ? (
                "Salvando..."
              ) : (
                <>
                  Cadastrar empresa <ChevronRight size={16} />
                </>
              )}
            </button>
          </footer>
