  ...
  return <div className="page">
    ...
    {view === "companies" ? <section className="panel data-panel">
      ...
      <div className="company-grid">{companies.map(company => <article className="company-card" key={company.id}>
        ...
        <StatusBadge tone="green">Ativa</StatusBadge><h2>{company.tradeName}</h2><p>{company.legalName}</p><dl><div><dt>CNPJ</dt><dd>{company.document}</dd></div><div><dt>Sede</dt><dd>{company.city}/{company.state}</dd></div></dl>
      ...
      <div className="org-tree">{data.companies.map(company => <div className="tree-company" key={company.id}>
        <div className="tree-node company"><Building2 /><span><strong>{company.tradeName}</strong><small>{company.document}</small></span><StatusBadge tone="green">Ativa</StatusBadge></div>
    ...
    <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Cadastrar empresa" description="Inclua a pessoa jurídica principal. Unidades e departamentos poderão ser adicionados em seguida.">
      <form className="form-grid" onSubmit={submit}>
        ...
        <label>CNPJ<input required value={form.document} onChange={e => setForm({ ...form, document: e.target.value })} placeholder="00.000.000/0000-00" /></label>
        ...
        <footer className="form-actions span-2"><button type="button" className="secondary-button" onClick={() => setModalOpen(false)}>Cancelar</button><button className="primary-button" disabled={mutation.isPending}>{mutation.isPending ? "Salvando..." : <>Cadastrar empresa <ChevronRight size={16} /></>}</button></footer>
      </form>
    </Modal>
  </div>;
}
