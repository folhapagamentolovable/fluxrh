...
import { BriefcaseBusiness, ChevronRight, Filter, Plus, Search, UserCheck, UsersRound } from "lucide-react";
...
export function EmployeesPage() {
  ...
  const { data: organization } = useQuery({ queryKey: ["organizations"], queryFn: getOrganizations });
  const [query, setQuery] = useState(""); const [status, setStatus] = useState("all"); const [modalOpen, setModalOpen] = useState(false); const [form, setForm] = useState(initialForm);
  ...
  return <div className="page">
    ...
    <section className="people-summary"><div><span className="metric-icon blue"><UsersRound /></span><strong>{employees.length}</strong><small>Total cadastrado</small></div><div><span className="metric-icon green"><UserCheck /></span><strong>{employees.filter(x => x.status === "active").length}</strong><small>Ativos</small></div><div><span className="metric-icon purple"><BriefcaseBusiness /></span><strong>{employees.filter(x => x.status === "onboarding").length}</strong><small>Em onboarding</small></div></section>
    <section className="panel data-panel">
      <div className="table-toolbar people-toolbar"><div className="field"><Search size={17} /><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Buscar nome, matrícula ou cargo" /></div><select value={status} onChange={e => setStatus(e.target.value)}><option value="all">Todos os status</option><option value="active">Ativos</option><option value="onboarding">Onboarding</option><option value="vacation">Em férias</option><option value="leave">Afastados</option></select><button className="secondary-button"><Filter size={16} /> Mais filtros</button></div>
      {isLoading ? <div className="table-loading">Carregando colaboradores...</div> : <div className="data-table-wrap"><table className="data-table"><thead><tr><th>Colaborador</th><th>Cargo e área</th><th>Empresa</th><th>Admissão</th><th>Status</th><th /></tr></thead><tbody>{filtered.map(employee => { const [label, tone] = statusMap[employee.status]; return <tr key={employee.id}><td><Link className="person-cell" to={`/pessoas/${employee.id}`}><span className="person-avatar" style={{ background: employee.avatarColor }}>{employee.fullName.split(" ").map(x => x[0]).slice(0,2).join("")}</span><span><strong>{employee.fullName}</strong><small>Matrícula {employee.registration}</small></span></Link></td><td><strong>{employee.position}</strong><small>{employee.departmentName}</small></td><td><strong>{employee.companyName}</strong><small>{employee.establishmentName}</small></td><td>{new Date(`${employee.hireDate}T12:00:00`).toLocaleDateString("pt-BR")}</td><td><StatusBadge tone={tone === "purple" ? "blue" : tone}>{label}</StatusBadge></td><td><Link className="row-arrow" to={`/pessoas/${employee.id}`}><ChevronRight size={18} /></Link></td></tr>})}</tbody></table>{filtered.length === 0 && <div className="table-empty">Nenhum colaborador encontrado.</div>}</div>}
    </section>
    <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Novo colaborador" description="Crie o prontuário inicial e prepare o workflow de admissão.">
      <form className="form-grid employee-form" onSubmit={submit}>
        ...
        <label>Departamento<select value={form.departmentId} onChange={e => setForm({...form,departmentId:e.target.value})}>{selectedUnits.filter(x => x.type === "department").map(x => <option key={x.id} value={x.id}>{x.name}</option>)}</select></label>
        <label>Centro de custo<select value={form.costCenterId} onChange={e => setForm({...form,costCenterId:e.target.value})}>{selectedUnits.filter(x => x.type === "cost_center").map(x => <option key={x.id} value={x.id}>{x.name}</option>)}</select></label>
