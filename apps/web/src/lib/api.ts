import { absenceOverviewSchema, admissionListSchema, admissionSchema, announcementSchema, benefitEnrollmentSchema, benefitsOverviewSchema, communicationsOverviewSchema, dashboardSnapshotSchema, documentOverviewSchema, documentRecordSchema, employeeListSchema, employeeMovementSchema, employeePortalOverviewSchema, employeeSchema, employeeTimeSummarySchema, medicalCertificateSchema, notificationSchema, organizationSnapshotSchema, payrollEmployeeSchema, payrollOverviewSchema, payrollRunSchema, serviceRequestSchema, specialCalculationSchema, specialCalculationsOverviewSchema, terminationProcessSchema, terminationsOverviewSchema, timeExceptionSchema, timeOverviewSchema, timePunchSchema, vacationRequestSchema, workflowOverviewSchema, type AbsenceOverview, type Admission, type Announcement, type ApiResponse, type BenefitEnrollment, type BenefitsOverview, type CommunicationsOverview, type CreateAdmissionInput, type CreateAnnouncementInput, type CreateBenefitEnrollmentInput, type CreateCompanyInput, type CreateDocumentRequestInput, type CreateEmployeeInput, type CreateEmployeeMovementInput, type CreateMedicalCertificateInput, type CreateServiceRequestInput, type CreateSpecialCalculationInput, type CreateTerminationInput, type CreateVacationRequestInput, type DashboardSnapshot, type DocumentOverview, type DocumentRecord, type Employee, type EmployeeListItem, type EmployeeMovement, type EmployeePortalOverview, type EmployeeTimeSummary, type MedicalCertificate, type Notification, type OrganizationSnapshot, type PayrollEmployee, type PayrollOverview, type PayrollRun, type ServiceRequest, type SpecialCalculation, type SpecialCalculationsOverview, type TerminationProcess, type TerminationsOverview, type TimeException, type TimeOverview, type TimePunch, type VacationRequest, type WorkflowOverview } from "@fluxrh/contracts";
import { employeeDependentSchema, timeCompetenceClosureSchema, type CreateEmployeeDependentInput, type EmployeeDependent, type TimeCompetenceClosure } from "@fluxrh/contracts";
...
export const getCommunications=():Promise<CommunicationsOverview>=>request("/api/v1/communications/overview",communicationsOverviewSchema);
export const markNotificationRead=(id:string):Promise<Notification>=>request(`/api/v1/communications/notifications/${id}/read`,notificationSchema,{method:"POST",body:"{}"});
export const acknowledgeNotification=(id:string):Promise<Notification>=>request(`/api/v1/communications/notifications/${id}/acknowledge`,notificationSchema,{method:"POST",body:"{}"});
export const createAnnouncement=(input:CreateAnnouncementInput):Promise<Announcement>=>request("/api/v1/communications/announcements",announcementSchema,{method:"POST",body:JSON.stringify(input)});
export const runCommunicationEscalations=()=>request("/api/v1/communications/escalations/run",notificationSchema,{method:"POST",body:"{}"});
export const getAnalytics=(filter:AnalyticsFilter={}):Promise<AnalyticsOverview>=>{const query=new URLSearchParams(Object.entries(filter).filter(([,v])=>Boolean(v)) as [string,string][]).toString();return request(`/api/v1/analytics/overview${query?`?${query}`:""}`,analyticsOverviewSchema)};
export const generateReport=(input:GenerateReportInput):Promise<ReportRun>=>request("/api/v1/analytics/reports/generate",reportRunSchema,{method:"POST",body:JSON.stringify(input)});
export const getOccupationalHealth=():Promise<OccupationalHealthOverview>=>request("/api/v1/occupational-health/overview",occupationalHealthOverviewSchema);
...
export const getCurrentTimeCompetence=():Promise<TimeCompetenceClosure|null>=>request("/api/v1/people/competences/current",timeCompetenceClosureSchema.nullable());
export const closeTimeCompetence=(id:string,notes?:string):Promise<TimeCompetenceClosure>=>request("/api/v1/people/competences/close",timeCompetenceClosureSchema,{method:"POST",body:JSON.stringify({id,notes})});
export const getEmployeeDependents=(employeeId:string):Promise<EmployeeDependent[]>=>request(`/api/v1/people/${employeeId}/dependents`,employeeDependentSchema.array());
export const createEmployeeDependent=(employeeId:string,input:CreateEmployeeDependentInput):Promise<EmployeeDependent>=>request(`/api/v1/people/${employeeId}/dependents`,employeeDependentSchema,{method:"POST",body:JSON.stringify({...input,employeeId})});
