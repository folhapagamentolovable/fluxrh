import {
  ...
  reviewMedicalCertificateSchema,
...
const memoryRepository = new InMemoryAbsencesRepository();
const repositoryFor = (authorization?: string) => getPersistenceMode() === "supabase"
  ? createPersistentModuleRepository(createRequestSupabaseClient(authorization), "absences", () => new InMemoryAbsencesRepository())
  : memoryRepository;
export async function absencesRoutes(app: FastifyInstance) {
  ...
  app.post("/vacations", async (req, reply) => {
    const p = createVacationRequestSchema.safeParse(req.body);
    if (!p.success)
      return reply
        .code(400)
        .send({ error: "validation_error", issues: p.error.issues });
    const result = await repositoryFor(req.headers.authorization).createVacation(p.data);
  ...
  app.post<{ Params: { id: string } }>(
    ...
    async (req, reply) => {
      const p = decideVacationRequestSchema.safeParse(req.body);
      if (!p.success)
        return reply.code(400).send({ error: "validation_error" });
      const value = await repositoryFor(req.headers.authorization).decideVacation(
        req.params.id,
        p.data.decision,
        p.data.note,
      );
  ...
  app.post("/certificates", async (req, reply) => {
    const p = createMedicalCertificateSchema.safeParse(req.body);
    if (!p.success)
      return reply
        .code(400)
        .send({ error: "validation_error", issues: p.error.issues });
    return sendData(reply, await repositoryFor(req.headers.authorization).createCertificate(p.data), 201);
  });
  app.post<{ Params: { id: string } }>(
    "/certificates/:id/review",
    async (req, reply) => {
      const p = reviewMedicalCertificateSchema.safeParse(req.body);
      if (!p.success)
        return reply.code(400).send({ error: "validation_error" });
      const value = await repositoryFor(req.headers.authorization).reviewCertificate(
        req.params.id,
        p.data.decision,
        p.data.note,
      );
