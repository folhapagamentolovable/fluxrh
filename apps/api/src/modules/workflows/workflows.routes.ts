...
export async function workflowsRoutes(app: FastifyInstance) {
  ...
  app.post("/admissions", async (request, reply) => { const parsed = createAdmissionSchema.safeParse(request.body); return parsed.success ? sendData(reply, await repository.create(normalizePersonalData(parsed.data)), 201) : reply.code(400).send({ error: "validation_error", issues: parsed.error.issues }); });
