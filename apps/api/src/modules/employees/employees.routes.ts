import { createEmployeeSchema, updateEmployeeSchema } from "@fluxrh/contracts";
...
function repositoryFor(authorization?: string): EmployeesRepository {
  return getPersistenceMode() === "supabase"
    ? new SupabaseEmployeesRepository(
...
export async function employeesRoutes(app: FastifyInstance) {
  app.get<{
    ...
    if (
      ...
      (request.query.status &&
        !statuses.includes(request.query.status as (typeof statuses)[number]))
    )
    ...
    return sendData(
      reply,
      await repositoryFor(request.headers.authorization).list({
        query: request.query.q?.slice(0, 100),
        status,
        limit,
        offset,
      }),
  ...
  app.get<{ Params: { id: string } }>("/:id", async (request, reply) => {
    const employee = await repositoryFor(
  ...
  app.post("/", async (request, reply) => {
    ...
    return sendData(
      reply,
      await repositoryFor(request.headers.authorization).create(
  ...
  app.patch<{ Params: { id: string } }>("/:id", async (request, reply) => {
    ...
    if (!parsed.success)
      return reply
        .code(400)
        .send({ error: "validation_error", issues: parsed.error.issues });
    const employee = await repositoryFor(request.headers.authorization).update(
      request.params.id,
      normalizePersonalData(parsed.data),
    );
    return employee
      ? sendData(reply, employee)
