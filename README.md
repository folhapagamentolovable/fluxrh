...
O ambiente externo está habilitado para operações reais por meio de um gate persistido. Requisições mutáveis (`POST`, `PUT`, `PATCH` e `DELETE`) são autorizadas exclusivamente quando a sessão autenticada possui vínculo ativo `super_admin`; os demais papéis permanecem somente leitura nesse ambiente.
