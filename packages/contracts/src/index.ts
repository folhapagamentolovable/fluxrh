...
export const analyticsPeriodPointSchema=z.object({period:z.string(),headcount:z.number(),hires:z.number(),terminations:z.number(),absenteeismRate:z.number(),payrollCost:z.number(),automationRate:z.number()});
export const analyticsDepartmentSchema=z.object({id:z.string(),name:z.string(),headcount:z.number(),payrollCost:z.number(),absenceHours:z.number(),overtimeHours:z.number(),openExceptions:z.number(),automationRate:z.number()});
export const managementInsightSchema=z.object({id:z.string(),title:z.string(),description:z.string(),severity:z.enum(["positive","attention","critical"]),metric:z.string(),change:z.number(),recommendation:z.string(),source:z.string()});
export const reportDefinitionSchema=z.object({id:z.string(),name:z.string(),description:z.string(),category:z.enum(["people","time","absence","payroll","automation","compliance"]),formats:z.array(z.enum(["csv","pdf"])),lastGeneratedAt:z.string().optional(),scheduled:z.boolean()});
export const reportRunSchema=z.object({id:z.string(),reportId:z.string(),reportName:z.string(),format:z.enum(["csv","pdf"]),status:z.enum(["processing","ready","failed"]),requestedAt:z.string(),completedAt:z.string().optional(),fileName:z.string().optional(),rows:z.number().optional()});
export const analyticsOverviewSchema=z.object({filters:z.object({companies:z.array(z.object({id:z.string(),name:z.string()})),departments:z.array(z.object({id:z.string(),name:z.string()})),selectedCompanyId:z.string(),selectedDepartmentId:z.string(),period:z.string()}),summary:z.object({headcount:z.number(),headcountChange:z.number(),turnoverRate:z.number(),turnoverChange:z.number(),absenteeismRate:z.number(),absenteeismChange:z.number(),monthlyPayrollCost:z.number(),payrollChange:z.number(),automationRate:z.number(),automationChange:z.number(),openExceptions:z.number()}),trend:z.array(analyticsPeriodPointSchema),departments:z.array(analyticsDepartmentSchema),insights:z.array(managementInsightSchema),reports:z.array(reportDefinitionSchema),runs:z.array(reportRunSchema)});
export const analyticsFilterSchema=z.object({companyId:z.string().optional(),departmentId:z.string().optional(),period:z.string().optional()});
export const generateReportSchema=z.object({reportId:z.string(),format:z.enum(["csv","pdf"]),companyId:z.string().optional(),departmentId:z.string().optional(),period:z.string().optional()});
export type AnalyticsOverview=z.infer<typeof analyticsOverviewSchema>;export type AnalyticsFilter=z.infer<typeof analyticsFilterSchema>;export type ReportRun=z.infer<typeof reportRunSchema>;export type GenerateReportInput=z.infer<typeof generateReportSchema>;

export const occupationalExamTypeSchema=z.enum(["admission","periodic","return_to_work","risk_change","termination"]);
export const occupationalExamSchema=z.object({id:z.string(),employeeId:z.string(),employeeName:z.string(),registration:z.string(),companyName:z.string(),departmentName:z.string(),position:z.string(),type:occupationalExamTypeSchema,scheduledDate:z.string(),dueDate:z.string(),status:z.enum(["required","scheduled","completed","overdue","cancelled"]),result:z.enum(["pending","fit","unfit","fit_with_restrictions"]),clinicName:z.string().optional(),responsiblePhysician:z.string().optional(),asoDocumentId:z.string().optional(),validUntil:z.string().optional(),functionalRestriction:z.string().optional(),workflowStatus:z.enum(["waiting","released","exception"])});
export const occupationalRiskSchema=z.object({id:z.string(),position:z.string(),departmentName:z.string(),category:z.enum(["physical","chemical","biological","ergonomic","accident"]),agent:z.string(),exposure:z.enum(["low","medium","high"]),requiredExams:z.array(z.string()),controls:z.array(z.string()),active:z.boolean()});
export const occupationalProgramSchema=z.object({id:z.string(),type:z.enum(["PCMSO","PGR","LTCAT","OTHER"]),name:z.string(),companyName:z.string(),effectiveFrom:z.string(),validUntil:z.string(),status:z.enum(["valid","expiring","expired","draft"]),responsible:z.string(),documentStatus:z.enum(["available","pending","under_review"])});
export const occupationalExceptionSchema=z.object({id:z.string(),examId:z.string().optional(),employeeName:z.string().optional(),title:z.string(),description:z.string(),severity:z.enum(["critical","high","medium","low"]),status:z.enum(["open","resolved"]),owner:z.string(),dueAt:z.string(),resolutionNote:z.string().optional()});
export const occupationalHealthOverviewSchema=z.object({summary:z.object({complianceRate:z.number(),examsDue30Days:z.number(),overdueExams:z.number(),restrictedEmployees:z.number(),expiringPrograms:z.number(),openExceptions:z.number()}),exams:z.array(occupationalExamSchema),risks:z.array(occupationalRiskSchema),programs:z.array(occupationalProgramSchema),exceptions:z.array(occupationalExceptionSchema),calendar:z.array(z.object({date:z.string(),examId:z.string(),employeeName:z.string(),type:occupationalExamTypeSchema,status:z.string()}))});
export const createOccupationalExamSchema=z.object({employeeId:z.string(),employeeName:z.string().min(3),registration:z.string(),companyName:z.string().min(2),departmentName:z.string().min(2),position:z.string().min(2),type:occupationalExamTypeSchema,scheduledDate:z.string(),dueDate:z.string(),clinicName:z.string().optional()});
export const completeOccupationalExamSchema=z.object({result:z.enum(["fit","unfit","fit_with_restrictions"]),responsiblePhysician:z.string().min(3),asoDocumentId:z.string().min(3),validUntil:z.string(),functionalRestriction:z.string().max(500).optional()}).refine(x=>x.result!=="fit_with_restrictions"||Boolean(x.functionalRestriction),{message:"restriction_required"});
export const resolveOccupationalExceptionSchema=z.object({note:z.string().min(3).max(500)});
export type OccupationalExam=z.infer<typeof occupationalExamSchema>;export type OccupationalHealthOverview=z.infer<typeof occupationalHealthOverviewSchema>;export type CreateOccupationalExamInput=z.infer<typeof createOccupationalExamSchema>;export type CompleteOccupationalExamInput=z.infer<typeof completeOccupationalExamSchema>;

export const patrolPointSchema=z.object({id:z.string(),name:z.string(),locationName:z.string(),sequence:z.number().int().positive(),qrToken:z.string(),instructions:z.string(),latitude:z.number().optional(),longitude:z.number().optional(),active:z.boolean(),lastVisitAt:z.string().optional()});
export const patrolRouteSchema=z.object({id:z.string(),name:z.string(),siteName:z.string(),companyName:z.string(),shift:z.string(),estimatedMinutes:z.number().positive(),toleranceMinutes:z.number().nonnegative(),active:z.boolean(),assignedEmployees:z.array(z.string()),points:z.array(patrolPointSchema)});
export const patrolOccurrenceSchema=z.object({id:z.string(),patrolId:z.string(),pointId:z.string().optional(),type:z.enum(["open_door","lighting","damaged_equipment","unauthorized_access","identified_risk","critical_incident","other"]),title:z.string(),description:z.string(),severity:z.enum(["critical","high","medium","low"]),status:z.enum(["open","in_review","resolved"]),reportedAt:z.string(),reportedBy:z.string(),evidenceCount:z.number(),resolutionNote:z.string().optional()});
export const patrolVisitSchema=z.object({id:z.string(),pointId:z.string(),pointName:z.string(),sequence:z.number(),scannedAt:z.string(),source:z.enum(["qr_code","offline_sync","manual"]),deviceId:z.string(),locationValid:z.boolean(),status:z.enum(["valid","out_of_order","late","invalid_location"])});
export const patrolSchema=z.object({id:z.string(),routeId:z.string(),routeName:z.string(),siteName:z.string(),employeeId:z.string(),employeeName:z.string(),scheduledStart:z.string(),startedAt:z.string().optional(),completedAt:z.string().optional(),status:z.enum(["scheduled","in_progress","exception","completed","cancelled"]),progress:z.number().min(0).max(100),currentPointSequence:z.number().int().nonnegative(),visits:z.array(patrolVisitSchema),occurrences:z.array(patrolOccurrenceSchema),exceptions:z.array(z.object({id:z.string(),title:z.string(),description:z.string(),severity:z.enum(["critical","high","medium","low"]),status:z.enum(["open","resolved"])}))});
export const patrolOverviewSchema=z.object({summary:z.object({scheduledToday:z.number(),inProgress:z.number(),completedToday:z.number(),coverageRate:z.number(),openOccurrences:z.number(),openExceptions:z.number()}),routes:z.array(patrolRouteSchema),patrols:z.array(patrolSchema),occurrences:z.array(patrolOccurrenceSchema),recentVisits:z.array(patrolVisitSchema.extend({employeeName:z.string(),siteName:z.string()}))});
export const startPatrolSchema=z.object({employeeId:z.string(),employeeName:z.string().min(3),deviceId:z.string().min(3)});
export const registerPatrolVisitSchema=z.object({token:z.string().min(5),deviceId:z.string().min(3),offline:z.boolean().default(false),locationValid:z.boolean().default(true)});
export const createPatrolOccurrenceSchema=z.object({pointId:z.string().optional(),type:z.enum(["open_door","lighting","damaged_equipment","unauthorized_access","identified_risk","critical_incident","other"]),title:z.string().min(3),description:z.string().min(5).max(1000),severity:z.enum(["critical","high","medium","low"]),reportedBy:z.string().min(2),evidenceCount:z.number().int().min(0).max(20)});
...
export const permissionMatrixEntrySchema=z.object({role:governanceRoleSchema,module:governanceModuleSchema,actions:z.array(governanceActionSchema),dataAccess:z.enum(["none","own","team","scope","organization"]),sensitiveData:z.enum(["hidden","masked","visible"])});
export const auditEventSchema=z.object({id:z.string(),organizationId:z.string(),actorId:z.string(),actorName:z.string(),actorType:z.enum(["user","automation","system"]),action:z.string(),module:governanceModuleSchema,entityType:z.string(),entityId:z.string(),summary:z.string(),occurredAt:z.string(),origin:z.string(),ipAddress:z.string(),before:z.record(z.string(),z.unknown()).optional(),after:z.record(z.string(),z.unknown()).optional(),justification:z.string().optional(),risk:z.enum(["normal","sensitive","critical"])});
export const governanceSessionSchema=z.object({id:z.string(),userId:z.string(),userName:z.string(),device:z.string(),browser:z.string(),ipAddress:z.string(),location:z.string(),createdAt:z.string(),lastSeenAt:z.string(),current:z.boolean(),status:z.enum(["active","revoked","expired"])});
export const governanceOverviewSchema=z.object({summary:z.object({activeUsers:z.number(),pendingInvites:z.number(),activeSessions:z.number(),sensitiveActionsToday:z.number(),deniedAttempts:z.number(),mfaCoverage:z.number()}),users:z.array(governanceUserSchema),permissions:z.array(permissionMatrixEntrySchema),audit:z.array(auditEventSchema),sessions:z.array(governanceSessionSchema),policies:z.array(z.object({id:z.string(),name:z.string(),description:z.string(),status:z.enum(["active","attention"]),coverage:z.number()}))});
export const inviteGovernanceUserSchema=z.object({name:z.string().min(3),email:z.string().email(),role:governanceRoleSchema,companyIds:z.array(z.string()).min(1),departmentIds:z.array(z.string()),teamOnly:z.boolean()});
